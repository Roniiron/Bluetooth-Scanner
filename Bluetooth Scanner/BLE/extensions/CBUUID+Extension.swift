...
extension CBUUID {
    ...
    /// Returns a shortened, formatted UUID string
    /// For standard 16-bit UUIDs: returns the 4-character hex (e.g., "180A")
    /// For custom 128-bit UUIDs: returns formatted segments (e.g., "E112-8559-435F-5176")
    public var formattedString: String {
        let uuidString = self.uuidString
        
        // Check if it's a standard 16-bit UUID (4 hex characters)
        if uuidString.count == 4 {
            return uuidString.uppercased()
        }
        
        // For 128-bit UUIDs, format as segments
        if uuidString.count >= 19 {
